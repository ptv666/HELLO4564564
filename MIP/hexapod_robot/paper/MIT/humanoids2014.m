...
if nargin < 5
  use_symbolic = 0; %use_symbolic就是0
end
...
if (use_symbolic == 0 || use_symbolic == 2)
  t0 = tic();
  p2 = MixedIntegerFootstepPlanningProblem(biped, seed_plan, false);    %构造函数
  p2.weights = weights; %添加权重
  p2 = p2.addSinCosLinearEquality(false);   %添加sin和cos约束

  p2 = p2.addQuadraticRelativeObjective(false); %添加两步之间的约束
  p2 = p2.addZAndYawReachability(false);        %添加Z和Yaw角的两步可达性
  p2 = p2.addTrimToFinalPoses(false);           %添加Trim
  p2 = p2.addQuadraticGoalObjective(goal_pos, nsteps-1:nsteps, [1,1], false);   %添加最后一步与goal的二次代价约束
  p2 = p2.addTerrainRegions([], false);         %添加区域约束
  p2 = p2.addXYReachabilityCircles(false);      %添加xy的工作空间约束
  if use_symbolic == 2
