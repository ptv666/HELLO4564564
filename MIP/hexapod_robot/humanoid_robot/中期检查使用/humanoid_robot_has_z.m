classdef humanoid_robot_has_z<Gurobi_Interface
    %实际上和没有z的差别就只是在于矩阵的维度不同而已
    properties
        two_feet_distance = 0.5;        %定义两脚之间的距离
        delta_z = 0.15;     %两步之间z的高度差
        
        left_foot_init  = [0,0.25,0,0]';    %初始状态，加入z
        right_foot_init = [0,-0.25,0,0]';
        
        left_foot_goal  = [];   %目标状态从3×1变为了4×1
        right_foot_goal = [];
        
        %相关代价的权重--------似乎z的代价值很小，对于双足机器人是可以理解的，因为MIT论文中的实验都是在平面，或者实验的场地z由x,y决定了的，所以z可以小一点？
        weight_g = diag([100,100,100,20]);   %最后一步距离goal的权重
        weight_r = diag([10,10,10,5]);      %两步之间的权重--------%与goal相差大一点，这样确保能走到目标点
        weight_trim = 5;  %trim的代价值
%         weight_r_final = diag([0,0,0,0]);   %走最后一步的代价--------MIT定义的两步之间的权重特殊之处
        
        N = -1;                 %步数，必须为偶数   %在构造函数中确定
        min_steps = -1;         %trim估计数量使用
        feet_state_list = [];   %落足状态集合--------%是待优化项
        
        %*****先定义一个全局尺寸，之后再改****
        square_l = 0.8;     %一步工作空间尺寸，正方形
        delta_theta = 0.0;  %角度，可以计算出来约束住加速求解过程，也可以手动给定
        ground_size = 10;   %落足点决策变量的上下限
    ...
    methods
        ...
        function obj=humanoid_robot_has_z(goal)
            obj = obj@Gurobi_Interface();   %调用父类的构造函数
            
            %根据输入目标点提前确定可能的步数，创建足端序列变量，定义delta_theta
            obj.left_foot_goal  = [goal(1) - 0.5 * obj.two_feet_distance*sin(goal(4)),goal(2) + 0.5 * obj.two_feet_distance*cos(goal(4)), goal(3), goal(4)]';
            obj.right_foot_goal = [goal(1) + 0.5 * obj.two_feet_distance*sin(goal(4)),goal(2) - 0.5 * obj.two_feet_distance*cos(goal(4)), goal(3), goal(4)]';
            obj.N = norm([goal(1),goal(2)])/0.4;            %********--------0.4只是一个估计值
...
%             obj.delta_theta = ((goal(3)-0)/obj.N) * 1.02;   %在此处为了加入z，先实验验证
...
%             right_init_index = obj.vars.Feet_State_List.Index(:,2);
        ...
        function obj=set_Goal_cost(obj)
            ...
            Q(last_state_index_left,  last_state_index_left)  = obj.weight_g;    %4×4矩阵
            Q(last_state_index_right, last_state_index_right) = obj.weight_g;
            %-2x'Qxg
            c = zeros(obj.num_vars, 1);
            c(last_state_index_left)  = -2*obj.weight_g*obj.left_foot_goal;     %4×1矩阵
        ...
        function obj=set_Leg_Workspace_constrain(obj)
            ...
            %从xy再加入z时，依然认为xy决定了落足可行域，虽然对斜面问题而言不是很正确
            ...
            %加入delta_z的限制
            feet_index_z = obj.vars.Feet_State_List.Index(3,:);
            for i=3:obj.N
                A = zeros(1,obj.num_vars);
                b = zeros(1,1);
                A(1,feet_index_z(i))   = 1;
                A(1,feet_index_z(i-1)) = -1;
                b(1,1) = obj.delta_z;
                obj.addLinearConstraints_unequal(A,b,'z_1_con');    %绝对值的一侧
                obj.addLinearConstraints_unequal(-A,b,'z_2_con');   %绝对值的另一侧
            end
        ...
        function obj=set_Trim_flag(obj)
            ...
            for i=3:obj.N-2    %init和last已经被固定住了,它们对应的落足点坐标也不需要再被约束，尤其是init的落足点坐标 更是已经被确定了
                ...
                A(3, feet_Index(3,i)) = 1;	%对z
                A(3, feet_Index(3,j)) = -1;

                A(4, feet_Index(4,i)) = 1; 	%对yaw
                A(4, feet_Index(4,j)) = -1;
                
                A(1:4, trim_Index(i)) = 1 * M;	%当trim=1，则abs(delta)<=0，也就是和末状态是相同的
                b(1:4,1) = 1 * M;
                obj.addLinearConstraints_unequal(A, b, 'Trim_attribute_physics_Fir');
                A = -A;
                A(1:4,trim_Index(i)) = 1 * M;
        ...
        %添加落足点的region
        function obj=set_foot_region(obj,rectangle_region_con_has_z, init_region_num)
            ...
            for i=3:obj.N
                for j=1:num_region
                    ...
                    b = zeros(6,1);
                    A(1:2, feet_Index(1,i)) = rectangle_region_con_has_z(j).A(1:2);     %对x的约束
                    A(3:4, feet_Index(2,i)) = rectangle_region_con_has_z(j).A(3:4);     %对y的约束
                    A(5:6, feet_Index(3,i)) = rectangle_region_con_has_z(j).A(5:6);     %对z的约束
                    A(1:6,H_index(j,i)) = M;
                    b = rectangle_region_con_has_z(j).b + M;
