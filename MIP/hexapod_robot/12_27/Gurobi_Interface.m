...
%x'Qcx + q'x <= rhs         二次约束，QCP问题
  properties
    ...
    %创建多项式约束
    poly_con = [];  %     obj.exp_con = struct('xvar', {}, 'yvar', {}, 'p', {});
    
    %创建幂函数约束，幂函数的定义域只能是正数！！
    pow_con = [];   %     obj.exp_con = struct('xvar', {}, 'yvar', {}, 'a', {});
    
    %创建指数函数约束
    exp_con = [];   %     obj.exp_con = struct('xvar', {}, 'yvar', {}, 'funcpieces', {}, 'funcpiecelength', {}, 'funcpieceerror', {}, 'funcpieceratio', {});
  ...
  methods
    ...
    function obj = addVariable(obj, name, type_input, size_input, lb, ub, start_input)
      ...
      if isfield(obj.vars, name)
        error('变量不能重名。Class_Gurobi_Interface_Line_79');
      end
      if (size(size_input,1)~=1) || (size(size_input,2)~=2)
        error('变量的大小需要是1×2的，例如[2 3]。Class_Gurobi_Interface_Line_82');
      end
      ...
      if size(start_input, 1) > obj.vars.(name).size(1)
        error('初值行数量大于变量行数。Class_Gurobi_Interface_Line_112');
      end
      if size(start_input, 2) > obj.vars.(name).size(2)
        error('初值列数量大于变量列数。Class_Gurobi_Interface_Line_115');
      end
    ...
    function obj = addSinConstraint(obj,x_index,y_index)
      ...
      if (numel(x_index)~=1)|| (numel(y_index)~=1)
          error('添加y=sin(x)约束时，x和y的变量数目要相同.Class_Gurobi_Interface_Line_188');
      end
    ...
    function obj = addCosConstraint(obj,x_index,y_index)
        %x_index1为x变量矩阵中的索引，y_index为y变量矩阵中的索引
      if (numel(x_index)~=1)|| (numel(y_index)~=1)
          error('添加y=cos(x)约束时，x和y的变量数目要相同.Class_Gurobi_Interface_Line_195');
      end
    ...
    %添加y=p(x)多项式约束
    function obj = addPolyConstraint(obj,x_index,y_index,p)
         %x_index1为x变量矩阵中的索引，y_index为y变量矩阵中的索引
      if (numel(x_index)~=1)|| (numel(y_index)~=1)
          error('添加y=exp(x)约束时，需要一个一个添加.Class_Gurobi_Interface_Line_213');
      end
      new_poly_con = struct('xvar',x_index, 'yvar', y_index, 'p', p);
      obj.poly_con = [obj.poly_con,new_poly_con];
    end
    
    %添加y=pow(x,a)约束
    function obj = addPowConstraint(obj,x_index,y_index,a)
         %x_index1为x变量矩阵中的索引，y_index为y变量矩阵中的索引
      if (numel(x_index)~=1)|| (numel(y_index)~=1)
          error('添加y=exp(x)约束时，需要一个一个添加.Class_Gurobi_Interface_Line_208');
      end
      new_pow_con = struct('xvar',x_index, 'yvar', y_index, 'a', a);
      obj.pow_con = [obj.pow_con,new_pow_con];
    end
    
    %添加y=exp(x)约束
    function obj = addExpConstraint(obj,x_index,y_index)
         %x_index1为x变量矩阵中的索引，y_index为y变量矩阵中的索引
      if (numel(x_index)~=1)|| (numel(y_index)~=1)
          error('添加y=exp(x)约束时，需要一个一个添加.Class_Gurobi_Interface_Line_208');
      end
      new_exp_con = struct('xvar',x_index, 'yvar', y_index,  'funcpieces', -2);
      obj.exp_con = [obj.exp_con,new_exp_con];
    end
    ...
    function model = getGurobiModel(obj)
      ...
      %设置多项式函数约束
      if ~isempty(obj.poly_con)  %如果为空添加sin和cos约束会报错
          model.genconpoly = obj.poly_con;
      end
      
      %设置幂函数约束
      if ~isempty(obj.pow_con)  %如果为空添加sin和cos约束会报错
          model.genconpow = obj.pow_con;
      end
      
      %设置exp指数约束
      if ~isempty(obj.exp_con)  %如果为空添加sin和cos约束会报错
          model.genconexp = obj.exp_con;
      end
