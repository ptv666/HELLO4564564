classdef Quadruped_robot_shrink_T<Gurobi_Interface
    properties
...
%         body_height = 0.4;
        ...
        weight_r = diag([2,2,2,2]);     	%两步之间的权重--------%与goal相差大一点，这样确保能走到目标点
        weight_trim = 5;                    %trim的代价值，Trim和两步之间的代价值进行对抗
        ...
        delta_z_threshold = 0.01*0.999;     %高度变化阈值，危险指标，切换步态，乘以0.999是为了解决Gurobi没有严格的小于的问题，也就是解决等于threshold不定的问题
    ...
    methods
        ...
        function obj=Quadruped_robot_shrink_T(goal)
...
%             obj.min_steps = floor(temp_distance/obj.square_l);
        ...
        function obj=set_Goal_cost(obj)
            %获得最后状态向量在决策变量矩阵中的索引
            last_feet_states_Index = obj.vars.Feet_State_List.Index(:,end-3:end);
            for j=1:4
                %x'Qx
                Q = zeros(obj.num_vars, obj.num_vars);                              %二次目标矩阵Q需要完全构建，再相加
                Q(last_feet_states_Index(:,j),  last_feet_states_Index(:,j))  = obj.weight_g;	%4×4矩阵
                
                %-2x'Qxg
                c = zeros(obj.num_vars, 1);
                c(last_feet_states_Index(:,j),1)  = -2 * obj.weight_g * obj.feet_goal(:,j); 	%4×1矩阵
                
                %xg'Qxg
                alpha = obj.feet_goal(:,j)' * obj.weight_g * obj.feet_goal(:,j);	%优化目标常数
        ...
        function obj=set_Leg_Workspace_constrain(obj)
            ...
            sin_yaw_index = reshape(obj.vars.sin_yaw.Index,1, []);	%转成行向量
            cos_yaw_index = reshape(obj.vars.cos_yaw.Index,1, []);
            for i=1:obj.N
                obj.addSinConstraint(Feet_State_Index(4,i), sin_yaw_index(1,i));
                obj.addCosConstraint(Feet_State_Index(4,i), cos_yaw_index(1,i));
            end
...
%             end
            ...
            for i=5:obj.N-4     %init和last已经被固定住了,它们对应的落足点坐标也不需要再被约束，尤其是init的落足点坐标 更是已经被确定了
                ...
                A(1:4, trim_Index(1,i)) = 1 * M;	%当trim=1，则abs(delta)<=0，也就是和末状态是相同的
                b(1:4,1) = 1 * M;
...
%             obj.vars.H.ub(:,4) = 0; obj.vars.H.ub(init_region_num(4), 4) = 1;
            ...
            %对初始状态的落足区域任意取一个值即可
            obj.vars.H.lb(:,1) = 0; obj.vars.H.lb(1, 1) = 1;	%对1-左前腿
            obj.vars.H.ub(:,1) = 0; obj.vars.H.ub(1, 1) = 1;
            obj.vars.H.lb(:,2) = 0; obj.vars.H.lb(1, 2) = 1;	%对2-左后腿
            obj.vars.H.ub(:,2) = 0; obj.vars.H.ub(1, 2) = 1;
            obj.vars.H.lb(:,3) = 0; obj.vars.H.lb(1, 3) = 1;	%对3-右后腿
            obj.vars.H.ub(:,3) = 0; obj.vars.H.ub(1, 3) = 1;
            obj.vars.H.lb(:,4) = 0; obj.vars.H.lb(1, 4) = 1;	%对4-右前腿
            obj.vars.H.ub(:,4) = 0; obj.vars.H.ub(1, 4) = 1;
            ...
            for i=5:obj.N
                for j=1:num_region
                    ...
                    A(6,Feet_State_Index(2,i))    = QingXie_rectangle_region_con(j).A(end);   	%z和y之间的tan(theta)关系
                    A(1:6,H_index(j,i)) = M;
        ...
        %添加步态矩阵T，为每一步添加时刻->T矩阵变为shrink_T
        function obj=set_Gait_Matrix(obj)
            %添加步态矩阵T，一个周期一个周期的步态
            
            obj.addVariable('T', 'B', [4 obj.N], 0, 1);
            
            temp_init = repmat([1,0,0,0],4,1);
            %添加初始状态约束，只是约束住而已，并没有什么实际物理意义，因为初始状态不需要T来表示
            obj.vars.T.lb(1:4,1:4) = temp_init;	%初始状态对应的T无意义，任取满足“条件”的即可
            obj.vars.T.ub(1:4,1:4) = temp_init;
            
            obj.vars.T.lb(1,5) = 1;             %约束第一个运动步
            obj.vars.T.ub(1,5) = 1;
            
            T_Index = obj.vars.T.Index;         %获取T的索引
            
            %--------添加T的约束1：（基于数学）各行之和为1->一个周期的4×4矩阵每一行只有一个1
            for j=1:4:obj.N     %一个周期一个周期的看
                for i=1:4
                    Aeq = zeros(1,obj.num_vars);
                    beq = zeros(1,1);
                    Aeq(1,T_Index(i,j:j+3)) = 1;
                    beq(1,1) = 1;
                    obj.addLinearConstraints_equal(Aeq, beq, 'T_cycle_row_sum');
                end
            end     %和原版的约束数量相同
            
            %--------添加T的约束2：（基于物理）当前轮次执行的时间在上一轮次之后->shrink_T自动保证，但是也有必要加入Timing变量为了后期使用
            ...
            step_num = 0;
            for j=1:4:obj.N
                time_cycle = time_cycle + 4;
                for i=1:4
                    step_num = step_num + 1;
                    Aeq = zeros(1,obj.num_vars);
                    beq = zeros(1,1);
                    Aeq(1,T_Index(i,j:j+3)) = time_cycle;
                    Aeq(1,Timing_Index(1,step_num)) = -1;
                    beq(1,1) = 0;
                    obj.addLinearConstraints_equal(Aeq,beq,'Timing_compute');
                end
            end     %和原版的约束数量相同
            
            %--------添加T的约束3：（基于物理）避免不可能步态，顺撇与奔跑步态->shrink_T和原版相同
            for j=5:obj.N
                i = 1;
                A = zeros(4,obj.num_vars);
                b = zeros(4,1);
                ...
                obj.addLinearConstraints_unequal(A,b,'Gait_con'); %添加约束
            end     %约束数量为原来的N/4倍
        ...
        function obj=set_Equilibrium_cost(obj)
            ...
            obj.vars.T_column_sum.lb(1,1:4) = 0;
            obj.vars.T_column_sum.ub(1,1:4) = 0;
            ...
            for i=5:obj.N   %T矩阵每一列代价temp_init，不统计初始状态的
                Q = zeros(obj.num_vars,obj.num_vars);
                alpha = 0;
        ...
        function obj=set_Gait_switch(obj)
            ...
            for i=1:obj.N   %耗时0.1s
                ...
                Aeq(1,Sum_delta_z_Flag_Index(1,i)) = -1;
                beq(1,1) = 0;
            ...
            obj.vars.T_multiplication_delta_z_flag.lb(1:4,1:4) = 0;                 %对初始的前四个时刻约束为0，只需计算后面的时刻的状态转移
            obj.vars.T_multiplication_delta_z_flag.ub(1:4,1:4) = 0;
            T_new_Index = obj.vars.T_multiplication_delta_z_flag.Index;                   %获取T_new的索引矩阵
            ...
            for j=5:obj.N
                ...
                beq = zeros(1,1);
                if mod(j,4)==1
                    step_num_j = step_num_j + 4;
                end
                
                for i=1:4
                    step_num_i = step_num_j + i;
                    ...
                    A(2,Sum_delta_z_Flag_Index(1,step_num_i)) = -1;
...
%             obj.vars.moment_delta_z_flag.lb(1,1:4) = 0;
%             obj.vars.moment_delta_z_flag.ub(1,1:4) = 0;
...
%                 A(1,Column_Sum_New_T_Index(1,j)) = 1;
%                 A(1,moment_Flag_Index(1,j)) = -1 * M;
%                 b(1,1) = 0.5;
%                 %(0.5 - Sum_star) + M*new_Flag<=M
%                 A(2,Column_Sum_New_T_Index(1,j)) = -1;
%                 A(2,moment_Flag_Index(1,j)) = M;
%                 b(2,1) = M - 0.5;
%                 
%                 obj.addLinearConstraints_unequal(A, b, 'compue_time_delta_z_flag');
%             end
%             
%             %--------通过moment_delta_z_flag提取T的列和
%             obj.addVariable('sum_of_column_T', 'C', [1 obj.N], 0, 2);
%             obj.addVariable('wing_leg_num_in_danger', 'C' , [1 obj.N], 0, 2);
%             obj.vars.sum_of_column_T.lb(1,1:4) = 0;
%             obj.vars.sum_of_column_T.ub(1,1:4) = 0;
%             obj.vars.wing_leg_num_in_danger.lb(1,1:4) = 0;
%             obj.vars.wing_leg_num_in_danger.ub(1,1:4) = 0;
...
%             end
%             
%             for j=5:obj.N   %根据moment_delta_z_flag提取T的列和
%                 A = zeros(3,obj.num_vars);
%                 b = zeros(3,1);
%                 %abs(Sum - Sum_compute)<=(1 - Flag)M
%                 A(1,T_column_sum_Index(1,j)) = 1;
%                 A(1,Swing_Leg_num_Index(1,j)) = -1;
%                 A(1,moment_Flag_Index(1,j)) = M;
%                 b(1,1) = M;
%                 
%                 %绝对值的另外一侧
%                 A(2,T_column_sum_Index(1,j)) = -1;
%                 A(2,Swing_Leg_num_Index(1,j)) = 1;
%                 A(2,moment_Flag_Index(1,j)) = M;
%                 b(2,1) = M;
%                 
%                 %Sum_compute <= Flag*M
%                 A(3,Swing_Leg_num_Index(1,j)) = 1;
%                 A(3,moment_Flag_Index(1,j)) = -1 * M;
%                 b(3,1) = 0;
%                 
...
%             
% 
%             %--------如果delta_z超过了阈值，就对该状态转移的 “时刻” 摆动腿总数施加惩罚，(wing_leg_num_in_danger - 1)'Q(wing_leg_num_in_danger - 1)
%             %为wing_leg_num_in_danger添加二次代价值，并且在加入代价的时候，把摆动腿的数量减去1，否则in_danger时即便只有一条腿最终也会有很高的代价值
%             for j=5:obj.N
%                 Q = zeros(obj.num_vars,obj.num_vars);
%                 c = zeros(obj.num_vars,1);
%                 alpha = 0.0;
%                 Q(Swing_Leg_num_Index(1,j),Swing_Leg_num_Index(1,j)) = obj.weight_swing_leg_num_in_danger;  %wing_leg_num_in_danger'*Q*wing_leg_num_in_danger
%                 c(Swing_Leg_num_Index(1,j)) = -2*obj.weight_swing_leg_num_in_danger;                        %-2*wing_leg_num_in_danger*Q
%                 alpha = obj.weight_swing_leg_num_in_danger;                                                 %Q
%                 
